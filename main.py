...
def render_cards_selected(df: pd.DataFrame):
    for _, r in df.iterrows():
        ...
        title = f"{sym} / {strat}"
        items = []
        for k in ["現在値（終値）","Entry目安","SL目安","TP目安","RR","実質RR","TP根拠","SL根拠","トレンド監査","短期反転確認","Entry監査メモ","価格更新状態","価格更新メモ","再計算失敗フラグ","売買優先区分","実行優先帯","実行優先スコア","今すぐ発注スコア","単元予算可否","単元推奨可否","単元必要資金(円)","単元想定損失(円)","推奨株数","推奨投資額(円)","想定損失(円)","発注不可理由","selected_now判定","selected_now除外理由","selected_now空理由集計","総合スコア"]:
            if k in df.columns:
                v = r.get(k)
                items.append(f"{k}:{v}")
        st.markdown(
...
def render_cards_guide(df: pd.DataFrame):
    for _, r in df.iterrows():
        ...
        items = []
        for k in ["セクター","売買優先区分","実行優先帯","発注単位","単元予算可否","単元推奨可否","推奨株数","推奨投資額(円)","想定利益(円)","期待純益(円)","想定損失(円)","Entry目安","SL目安","TP目安","TP根拠","SL根拠","トレンド監査","短期反転確認","Entry監査メモ","最大保有","Entry状態"]:
            if k in df.columns:
...
def prepare_selected_view(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[schema]
    try:
        for c in ["企業名","セクター","推奨方式","売買優先区分","実行優先帯","発注単位","単元予算可否","単元推奨可否","単元予算判定理由","単元推奨判定理由","Entry状態","発注不可理由","価格更新状態","価格更新メモ","選定区分","selected_now判定","selected_now除外理由","selected_now空理由集計","TP根拠","SL根拠","トレンド監査","短期反転確認","Entry監査メモ"]:
            if c in df.columns:
                df[c] = (df[c].astype(str).replace(["None","none","nan","NaN",""], "不明" if c in ["企業名","セクター"] else "").str.strip())
        for c in ["現在値（終値）","Entry目安","SL目安","TP目安","利確利幅(円/株)","RR","実質RR","再計算失敗フラグ","単元必要資金(円)","単元想定損失(円)","推奨投資額(円)","想定利益(円)","期待純益(円)","想定損失(円)","総合スコア","実行優先度","実行優先スコア","今すぐ発注スコア","推奨株数","元Entry目安","元SL目安","元TP目安","元RR","元総合スコア","TP候補数","SL候補数"]:
            if c in df.columns:
...
def prepare_guide_view(df: pd.DataFrame, max_rows: int) -> pd.DataFrame:
    ...
    if isinstance(guide, pd.DataFrame) and len(guide):
        try:
            for c in ["銘柄","企業名","セクター","推奨方式","売買優先区分","実行優先帯","発注単位","単元予算可否","単元推奨可否","推奨株数","推奨投資額(円)","想定利益(円)","期待純益(円)","想定損失(円)","Entry目安","SL目安","TP目安","TP根拠","SL根拠","トレンド監査","短期反転確認","Entry監査メモ","最大保有","Entry状態"]:
                if c not in guide.columns:
                    guide[c] = None
            guide = guide[["銘柄","企業名","セクター","推奨方式","売買優先区分","実行優先帯","発注単位","単元予算可否","単元推奨可否","推奨株数","推奨投資額(円)","想定利益(円)","期待純益(円)","想定損失(円)","Entry目安","SL目安","TP目安","TP根拠","SL根拠","トレンド監査","短期反転確認","Entry監査メモ","最大保有","Entry状態"]]
            for c in ["企業名","セクター","推奨方式","売買優先区分","実行優先帯","発注単位","単元予算可否","単元推奨可否","Entry状態","TP根拠","SL根拠","トレンド監査","短期反転確認","Entry監査メモ"]:
...
st.markdown('<div class="small-note">最大保有10営業日 / TP・SL は直近高値安値・20/60/120/252日水準・移動平均・ラウンドナンバー・ATR安全幅を統合して再計算 — DB: Neon(Postgres)</div>', unsafe_allow_html=True)
...
if st.session_state.get("scan_results_ready", False):
    ...
    render_selected_section(
        "🟢 今すぐ発注ランキング",
        "単元株の発注圏を最優先にした即時発注候補です。追随可は、単元株の発注圏候補が0件のときに限り実質RR>=1.45を最大1件だけ補完します。S株は厳格条件の例外候補を優先し、それも無い場合のみ発注圏候補から最終補完を最大1件だけ許可します。空の場合は除外理由集計を上に表示します。",
        now_view,
        mobile_mode,
        show_top_n,
        "selected_now.csv",
