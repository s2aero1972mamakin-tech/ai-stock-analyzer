...
st.title("🤖 AI連携型 日本株 戦略分析ツール (S株対応版)")
...
with tab3:
    if st.button("💰 週末/月末 ポートフォリオ判断"):
        ...
        else: st.warning("API Keyを入力してください。")import streamlit as st
import plotly.graph_objects as go
from plotly.subplots import make_subplots
import pandas as pd
import math
...
ctx = {
    ...
    "rsi": latest["RSI"],
    "sma_diff": latest["SMA_DIFF"],
    "us10y": latest.get("BENCHMARK", 0.0), # 変数名は互換性のためus10yだが中身は日経平均
    "panel_short": diag['short']['status'],
    "panel_mid": diag['mid']['status']
}
