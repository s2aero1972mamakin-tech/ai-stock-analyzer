...
def scan_swing_candidates(
    ...
    # ---- 自動緩和（pullbackで0件なら breakoutへ自動切替 + 条件緩和 + セクター絞り込みOFFで再スキャン）
    # relax_level は内部で 0→1→2 と最大2回まで自動再試行します。
    max_auto_relax = 2
    base_params = params
    base_sector_prefilter = bool(sector_prefilter)
    relax_notes: List[Dict[str, object]] = []
    master_all = get_jpx_master()
    if master_all.empty:

    # ---- 自動緩和の判定 ----
    # pullbackで0件（prelim_pass=0）になりやすいので、まずは breakout に切替 + 条件緩和で再スキャン。
    # さらに0件なら、もう一段緩めて再スキャンします。
    try:
        prelim_pass = int(stats.get("prelim_pass", 0))
    except Exception:
        prelim_pass = int(len(prelim) if prelim is not None else 0)

    if prelim_pass <= 0 and int(relax_level) < int(max_auto_relax):
        next_level = int(relax_level) + 1

        # pullback→breakout は level1 で必ず実施（ユーザー要件）
        relaxed_params, note = _apply_auto_relax(params, level=next_level)
        relax_notes.append(note)

        # セクター絞り込みOFFで再スキャン（ユーザー要件）
        # ※ sector_prefilter=False にしてユニバースを最大化する
        return scan_swing_candidates(
            ...
            api_key=api_key,
            relax_level=next_level,
        )

    # attach relax notes (if any) for UI/debug
    try:
        if relax_notes:
            stats["auto_relax_notes"] = relax_notes
    except Exception:
        pass


        return {"regime_ok": True, "candidates": [], "prelim_count": 0, "bt_count": 0, "selected_sectors": [], "sector_ranking": [], "relax_level": int(relax_level), "params_effective": (params.__dict__ if hasattr(params, "__dict__") else {}), "filter_stats": {}, "universe": 0, "error": "JPXマスター取得に失敗しました"}
