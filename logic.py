...
# =============================================================================
# INSTITUTIONAL_QUANT_ENGINE_V11 (2ファイル統合・後方互換)
# =============================================================================
INSTITUTIONAL_QUANT_ENGINE_V11 = "2026-03-05"
# (以下、追加実装は v11 統合レイヤー。既存関数をラップして拡張)
...
# --- 市場レジームAI ---
def market_regime_ai(index_symbol: str = "^N225") -> Dict[str, Any]:
    try:
        hist = yf.download(index_symbol, period="2y", interval="1d", progress=False, auto_adjust=True)
        if hist is None or len(hist) < 60:
            return {"ok": False, "レジーム": "不明", "指数": index_symbol}
        c = hist["Close"].astype(float).dropna()
        r = c.pct_change().dropna()
        vol20 = float(r.rolling(20).std().iloc[-1]) if len(r) >= 25 else float(r.std())
        mom60 = float(c.iloc[-1]/c.iloc[-61]-1.0) if len(c) >= 61 else float(c.iloc[-1]/c.iloc[0]-1.0)
        ma50 = float(c.rolling(50).mean().iloc[-1])
        ma200 = float(c.rolling(200).mean().iloc[-1]) if len(c) >= 200 else float(c.rolling(120).mean().iloc[-1])
        px = float(c.iloc[-1])
        trend = 1 if (px > ma50 and ma50 > ma200) else (-1 if (px < ma50 and ma50 < ma200) else 0)
        high_vol = (np.isfinite(vol20) and vol20 > 0.02)
        if trend > 0 and mom60 > 0.05 and not high_vol:
            reg = "強気"
        elif trend < 0 and mom60 < -0.05 and not high_vol:
            reg = "弱気"
        elif high_vol and mom60 < 0:
            reg = "弱気（高ボラ）"
        elif high_vol:
            reg = "横ばい（高ボラ）"
        else:
            reg = "横ばい"
        return {"ok": True, "指数": index_symbol, "レジーム": reg, "モメンタム60日": mom60, "ボラ20日": vol20}
    except Exception as e:
        return {"ok": False, "レジーム": "不明", "指数": index_symbol, "error": repr(e)}

def _regime_multiplier(regime: str) -> float:
    if regime == "強気": return 1.08
    if regime == "横ばい": return 1.00
    if regime == "横ばい（高ボラ）": return 0.92
    if regime == "弱気": return 0.88
    if regime == "弱気（高ボラ）": return 0.80
    return 1.00

# --- セクターフローAI ---
def sector_flow_ai(price_df_long: pd.DataFrame, meta: pd.DataFrame) -> pd.DataFrame:
    if price_df_long is None or price_df_long.empty or meta is None or meta.empty:
        return pd.DataFrame()
    d = price_df_long.copy()
    if "Symbol" not in d.columns or "Date" not in d.columns or "Close" not in d.columns:
        return pd.DataFrame()
    meta2 = meta.copy()
    if "symbol" not in meta2.columns:
        return pd.DataFrame()
    meta2["sym_key"] = meta2["symbol"].astype(str).map(_sym_key)
    meta2["sector33_name"] = meta2.get("sector33_name", pd.Series(index=meta2.index, dtype=object)).fillna("不明")
    d["sym_key"] = d["Symbol"].astype(str).map(_sym_key)
    d = d.merge(meta2[["sym_key","sector33_name"]], on="sym_key", how="left")
    d["sector33_name"] = d["sector33_name"].fillna("不明")
    piv = d.pivot_table(index="Date", columns="sym_key", values="Close", aggfunc="last").sort_index()
    if piv.shape[0] < 40:
        return pd.DataFrame()
    ret1m = piv.pct_change(21).iloc[-1]
    ret3m = piv.pct_change(63).iloc[-1]
    key2sec = meta2.drop_duplicates("sym_key").set_index("sym_key")["sector33_name"].to_dict()
    df = pd.DataFrame({"sym_key": ret3m.index.astype(str), "3ヶ月リターン": ret3m.values, "1ヶ月リターン": ret1m.reindex(ret3m.index).values})
    df["セクター"] = df["sym_key"].map(key2sec).fillna("不明")
    g = df.groupby("セクター", dropna=False).agg({"3ヶ月リターン":"median","1ヶ月リターン":"median"}).reset_index()
    g["セクターフロー"] = g["3ヶ月リターン"] - g["1ヶ月リターン"]
    g["セクター強度"] = _zscore(g["3ヶ月リターン"]).values
    g["フロー強度"] = _zscore(g["セクターフロー"]).values
    g["セクター総合"] = 0.65*g["セクター強度"] + 0.35*g["フロー強度"]
    return g.sort_values("セクター総合", ascending=False).reset_index(drop=True)

def _top_sectors_from_flow(flow_df: pd.DataFrame, top_n: int = 8) -> List[str]:
    if flow_df is None or flow_df.empty:
        return []
    return [str(x) for x in flow_df["セクター"].head(int(max(1,top_n))).tolist() if str(x).strip()]

# --- リスクパリティ / BL / MC ---
def _portfolio_weights_risk_parity(returns: pd.DataFrame) -> np.ndarray:
...
def _portfolio_sharpe(returns: pd.DataFrame, w: np.ndarray) -> float:
    if returns is None or returns.empty or w is None or len(w)==0:
        return np.nan
    ...
    if not np.isfinite(var_p) or var_p <= 1e-12:
        return np.nan
    return float(mu_p/np.sqrt(var_p))

def montecarlo_portfolio_risk(returns: pd.DataFrame, w: np.ndarray, n_sims: int = 2500, horizon_days: int = 60, ruin_dd: float = 0.25) -> Dict[str, Any]:
    if returns is None or returns.empty or w is None or len(w)==0:
        return {"ok": False}
    R = returns.dropna()
    if len(R) < 40:
        return {"ok": False, "reason":"returns_short"}
    mu = R.mean().values
    cov = R.cov().values
    cov = np.nan_to_num(cov, nan=0.0, posinf=0.0, neginf=0.0) + np.eye(len(mu))*1e-8
    try:
        L = np.linalg.cholesky(cov); use_chol=True
    except Exception:
        vals, vecs = np.linalg.eigh(cov)
        vals = np.clip(vals, 0.0, None)
        L = vecs @ np.diag(np.sqrt(vals)); use_chol=False
    n = len(mu); h=int(max(10,horizon_days)); sims=int(max(500,n_sims))
    z = np.random.normal(size=(sims,h,n))
    simR = z @ L.T + mu
    port_r = simR @ w
    eq = np.cumprod(1.0+port_r, axis=1)
    peak = np.maximum.accumulate(eq, axis=1)
    dd = 1.0 - eq/np.maximum(peak,1e-12)
    max_dd = dd.max(axis=1)
    ruin_prob = float((max_dd >= ruin_dd).mean())
    return {"ok": True,"n_sims": sims,"horizon_days": h,"cholesky": use_chol,"期待リターン": float(eq[:,-1].mean()-1.0),
            "最大DD_P50": float(np.quantile(max_dd,0.50)),"最大DD_P95": float(np.quantile(max_dd,0.95)),
            "破産確率": ruin_prob,"破産DD閾値": float(ruin_dd)}

def black_litterman_weights(returns: pd.DataFrame, views_mu: pd.Series, tau: float = 0.05, risk_aversion: float = 2.5) -> np.ndarray:
    if returns is None or returns.empty:
        return np.array([])
    R = returns.dropna()
    if len(R) < 60:
        return np.array([])
    cov = R.cov().values
    cov = np.nan_to_num(cov, nan=0.0, posinf=0.0, neginf=0.0) + np.eye(cov.shape[0])*1e-8
    n = cov.shape[0]
    w_mkt = np.ones(n)/n
    pi = risk_aversion*(cov @ w_mkt)
    v = views_mu.reindex(R.columns).fillna(0.0).values
    P = np.eye(n); Q = v
    omega = np.diag(np.diag(P @ (tau*cov) @ P.T))
    inv = np.linalg.inv(np.linalg.inv(tau*cov) + P.T @ np.linalg.inv(omega) @ P)
    mu_bl = inv @ (np.linalg.inv(tau*cov) @ pi + P.T @ np.linalg.inv(omega) @ Q)
    try:
        w = np.linalg.solve(risk_aversion*cov, mu_bl)
    except Exception:
        w = np.linalg.pinv(risk_aversion*cov) @ mu_bl
    w = np.maximum(w,0.0)
    s = w.sum()
    return (w/s) if s>0 else (np.ones(n)/n)

# --- 自己進化パラメータ（kv_meta） ---
def _load_evo_params() -> Dict[str, Any]:
    try:
        conn = _connect()
        try:
            raw = _get_meta(conn, "evo_params_v11")
        finally:
            conn.close()
        if raw:
        ...
        pass
    return {"w_ev":0.55,"w_trend":0.15,"w_wf":0.15,"w_risk":0.15,"top_sector_n":10}

def _save_evo_params(p: Dict[str, Any]) -> None:
    try:
        conn = _connect()
        try:
            _set_meta(conn, "evo_params_v11", json.dumps(p, ensure_ascii=False, default=_json_safe))
            conn.commit()
        finally:
            conn.close()
    except Exception:
...
def _evo_update_from_diag(diag: Dict[str, Any]) -> None:
    try:
        p = _load_evo_params()
        wf_win = _safe_float(diag.get("wf_oos_winrate_mean"), np.nan)
        mc_dd = _safe_float(diag.get("mc_dd5_mean"), np.nan)
        lr = 0.02
        if np.isfinite(wf_win):
            delta = (0.55 - wf_win)
            p["w_risk"] = _clip01(float(p["w_risk"]) + lr*delta)
            p["w_wf"] = _clip01(float(p["w_wf"]) + lr*(-delta))
        if np.isfinite(mc_dd):
            p["w_risk"] = _clip01(float(p["w_risk"]) + lr*(mc_dd - 0.05))
        s = float(p["w_ev"])+float(p["w_trend"])+float(p["w_wf"])+float(p["w_risk"])
        if s <= 0: s = 1.0
        for k in ["w_ev","w_trend","w_wf","w_risk"]:
            p[k] = float(p[k])/s
        _save_evo_params(p)
    except Exception:
        pass

# --- stage0_select をセクター強弱で絞り込み ---
_stage0_select_base = stage0_select
def stage0_select(min_price: float, min_avg_volume: float, keep: int) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
    s0, sec, meta = _stage0_select_base(min_price=min_price, min_avg_volume=min_avg_volume, keep=keep)
    try:
        evo = _load_evo_params()
        top_n = int(evo.get("top_sector_n", 10))
        sec_col = next((c for c in ["sector33_name","セクター","sector","Sector"] if c in s0.columns), None)
        if sec_col is None:
            return s0, sec, meta
        meta_df = universe_load_meta()
        syms = s0["symbol"].astype(str).map(_norm_symbol).tolist() if "symbol" in s0.columns else []
        syms = [s for s in syms if s]
        px = fetch_last_n_days(syms, n_days=110)
        flow = sector_flow_ai(px, meta_df)
        top_secs = _top_sectors_from_flow(flow, top_n=top_n)
        if top_secs:
            s0 = s0[s0[sec_col].fillna("不明").astype(str).isin(set(top_secs))].copy().head(int(keep))
            meta = dict(meta) if isinstance(meta, dict) else {}
            meta["sector_flow"] = flow
            meta["top_sectors"] = top_secs
        return s0, sec, meta
    except Exception:
        return s0, sec, meta

# --- run_scan_3stage を拡張 ---
_run_scan_3stage_base = run_scan_3stage
def run_scan_3stage(*args, **kwargs) -> Dict[str, Any]:
    out = _run_scan_3stage_base(*args, **kwargs)
    try:
        if not isinstance(out, dict) or not out.get("ok", False):
            return out
        diag = out.get("diag", {}) or {}
        evo = _load_evo_params()
        reg = market_regime_ai("^N225")
        diag["市場レジーム"] = reg.get("レジーム","不明")
        mult = _regime_multiplier(diag["市場レジーム"])
        diag["市場レジーム係数"] = mult

        meta = out.get("meta", {})
        if isinstance(meta, dict) and isinstance(meta.get("sector_flow"), pd.DataFrame):
            out["sector_flow"] = meta.get("sector_flow")

        df = out.get("selected")
        if isinstance(df, pd.DataFrame) and len(df):
            d = df.copy()
            if "銘柄" in d.columns and "symbol" not in d.columns: d["symbol"]=d["銘柄"]
            if "銘柄名" in d.columns and "name" not in d.columns: d["name"]=d["銘柄名"]
            if "セクター" in d.columns and "sector33_name" not in d.columns: d["sector33_name"]=d["セクター"]

            syms = d["symbol"].astype(str).map(_norm_symbol).tolist()
            px = fetch_last_n_days(syms, n_days=260)
            if px is not None and not px.empty:
                piv = px.pivot_table(index="Date", columns="Symbol", values="Close", aggfunc="last").sort_index()
                rets = piv.pct_change().dropna(how="all").replace([np.inf,-np.inf], np.nan).dropna(axis=1, how="any")
                sharpe = (rets.mean()/(rets.std(ddof=0)+1e-12)).replace([np.inf,-np.inf], np.nan)
                d["シャープ"] = d["symbol"].map(sharpe.to_dict())
                views = (pd.to_numeric(d.set_index("symbol")["3ヶ月リターン"], errors="coerce").fillna(0.0)/63.0) if "3ヶ月リターン" in d.columns else rets.mean().reindex(rets.columns).fillna(0.0)
                w_rp = _portfolio_weights_risk_parity(rets)
                w_bl = black_litterman_weights(rets, views if isinstance(views, pd.Series) else pd.Series(0.0, index=rets.columns))
                s_rp = _portfolio_sharpe(rets, w_rp) if len(w_rp) else np.nan
                s_bl = _portfolio_sharpe(rets, w_bl) if len(w_bl) else np.nan
                if np.isfinite(s_bl) and (not np.isfinite(s_rp) or s_bl >= s_rp):
                    w = w_bl; diag["ポートフォリオ方式"]="Black-Litterman（長期+見通し）"; diag["ポートフォリオシャープ"]=float(s_bl)
                else:
                    w = w_rp; diag["ポートフォリオ方式"]="リスクパリティ（相関分散最適化）"; diag["ポートフォリオシャープ"]=float(s_rp) if np.isfinite(s_rp) else None
                mc = montecarlo_portfolio_risk(rets, w, n_sims=2500, horizon_days=60, ruin_dd=0.25)
                out["portfolio_mc"] = mc
                if mc.get("ok"):
                    diag["破産確率(60日/25%DD)"] = mc.get("破産確率")
                    diag["最大DD_P95(60日)"] = mc.get("最大DD_P95")
                w_map = {sym: float(w[i]) for i, sym in enumerate(rets.columns)} if len(w) else {}
                d["推奨配分"] = d["symbol"].map(w_map).fillna(0.0)
                d["平均相関"] = d["symbol"].map(rets.corr().abs().mean().to_dict())

            base_col = next((c for c in ["総合スコア","final_score","利確スコア","スコア"] if c in d.columns), None)
            base = pd.to_numeric(d[base_col], errors="coerce").fillna(0.0) if base_col else pd.Series(0.0, index=d.index)
            trend = pd.to_numeric(d.get("AIトレンド", d.get("シャープ", pd.Series(index=d.index))), errors="coerce")
            wf = pd.to_numeric(d.get("WF勝率（OOS）", d.get("wf_oos_winrate", pd.Series(index=d.index))), errors="coerce")
            rr = pd.to_numeric(d.get("WF損益比RR（OOS）", d.get("wf_oos_rr", pd.Series(index=d.index))), errors="coerce")
            wf_score = 0.6*_zscore(wf.fillna(wf.median())) + 0.4*_zscore(rr.fillna(rr.median()))
            mcdd = pd.to_numeric(d.get("MC DD 5%", d.get("mc_dd5", pd.Series(index=d.index))), errors="coerce").fillna(0.0)
            ac = pd.to_numeric(d.get("平均相関", pd.Series(index=d.index))), errors="coerce").fillna(0.0)
            risk_pen = 0.5*_zscore(mcdd) + 0.5*_zscore(ac)
            w_ev=float(evo.get("w_ev",0.55)); w_tr=float(evo.get("w_trend",0.15)); w_wf=float(evo.get("w_wf",0.15)); w_rk=float(evo.get("w_risk",0.15))
            d["最終AIスコア"] = (w_ev*_zscore(base) + w_tr*_zscore(trend.fillna(trend.median())) + w_wf*wf_score - w_rk*risk_pen) * mult

            if "総合スコア" not in d.columns and base_col: d["総合スコア"]=base
            if "推奨方式" not in d.columns and "strategy" in d.columns: d["推奨方式"]=d["strategy"]
            if "バフェットスコア" not in d.columns and "buffett_score" in d.columns: d["バフェットスコア"]=d["buffett_score"]

            d = d.rename(columns={"symbol":"銘柄","name":"企業名","sector33_name":"セクター"})
            cols = [c for c in ["銘柄","企業名","セクター","3ヶ月リターン","WF勝率（OOS）","WF損益比RR（OOS）","MC DD 5%","シャープ","平均相関","総合スコア","推奨方式","バフェットスコア","推奨配分","最終AIスコア"] if c in d.columns]
            disp = d[cols].copy()
            if "バフェットスコア" in disp.columns:
                disp["バフェットスコア"] = pd.to_numeric(disp["バフェットスコア"], errors="coerce")
            disp = _clean_display_df(disp)
            if "最終AIスコア" in disp.columns:
                disp = disp.sort_values("最終AIスコア", ascending=False).reset_index(drop=True)
            out["selected"] = disp

        _evo_update_from_diag(diag)
        out["diag"] = diag
        return out
    except Exception as e:
        try:
            out.setdefault("errors", []).append({"where":"v11_post","error":repr(e)})
        except Exception:
            pass
        return out
