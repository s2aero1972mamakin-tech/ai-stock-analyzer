...
def _bump_reason(diag: Optional[dict], reason: str) -> None:
    """fail_data_reason を理由別に加算（後方互換・存在しなければ作成）"""
    if not diag:
        return
    try:
        r = str(reason)
        d = diag.get("fail_data_reason")
        if not isinstance(d, dict):
            d = {}
        d[r] = int(d.get(r, 0)) + 1
        diag["fail_data_reason"] = d
    except Exception:
        pass

def _http_get_csv(url: str, timeout: int = 25, retries: int = 3, diag: Optional[dict] = None, ticker: str = "") -> Optional[str]:
    ...
    for _ in range(retries):
        ...
        try:
            ...
            if diag is not None:
                ...
                timing["fetch_sec"] = float(timing.get("fetch_sec", 0.0)) + float(fetch_sec)
                diag["timing"] = timing
                if ticker:
                    diag["last_ticker"] = str(ticker)

            if r.status_code in (429, 503, 502):
                _bump_reason(diag, f"http_{r.status_code}")
                time.sleep(backoff)
            ...
            if "Date,Open,High,Low,Close" not in text[:300]:
                _bump_reason(diag, "bad_header")
                return None
            return text
        except requests.exceptions.Timeout:
            _bump_reason(diag, "timeout")
            time.sleep(backoff)
            backoff = min(8.0, backoff * 2)
        except Exception:
            _bump_reason(diag, "fetch_error")
            time.sleep(backoff)
            backoff = min(8.0, backoff * 2)
    return None

def get_market_data(ticker: str, period: str = "2y", interval: str = "1d", diag: Optional[dict] = None) -> pd.DataFrame:
    sym = _stooq_symbol(ticker)
    for dom in _STOOQ_DOMAINS:
        url = f"https://{dom}/q/d/l/?s={sym}&i=d"
        text = _http_get_csv(url, diag=diag, ticker=ticker)
        if not text:
            continue
        try:
            ...
            parse_sec = time.time() - t_parse0
            if diag is not None:
                timing = diag.get('timing') if isinstance(diag.get('timing'), dict) else {}
                timing['parse_sec'] = float(timing.get('parse_sec', 0.0)) + float(parse_sec)
                diag['timing'] = timing
            if df.empty:
                _bump_reason(diag, 'empty_data')
                continue
            df.columns = [c.strip().capitalize() for c in df.columns]
            if "Close" not in df.columns:
                _bump_reason(diag, "missing_close")
                continue
...
def scan_swing_candidates(
    ...
    for i, t in enumerate(tickers, start=1):
        if i < int(start_index) + 1:
            continue
        if diag is not None:
            diag['cursor_index'] = int(i - 1)
            diag['updated_at'] = str(pd.Timestamp.now())
            diag['status'] = 'running'
            diag['stage'] = 'scanning'
        ...
        ind_sec = time.time() - t_ind0
        if diag is not None:
            timing = diag.get('timing') if isinstance(diag.get('timing'), dict) else {}
            timing['indicators_sec'] = float(timing.get('indicators_sec', 0.0)) + float(ind_sec)
            diag['timing'] = timing
        if ind is None or ind.empty:
            _bump_reason(diag, 'indicators_empty')
